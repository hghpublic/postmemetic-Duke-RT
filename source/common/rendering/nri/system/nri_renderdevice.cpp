...
bool NRIRenderDevice::RenderPathTracedScene(HWDrawInfo& di, int drawmode, bool portal)
{
	...
	if (rendered && (PerfLoopTraceActive() || (bool)nri_pt360absenceprobe || (bool)nri_ptfiltercompare))
	{
		...
		Printf(
			...
			shell.persistentVoxelMaterialPresentationAttempts,
			shell.persistentVoxelMaterialPresentationHits,
			shell.persistentVoxelMaterialPresentationMisses,
			shell.persistentVoxelMaterialPresentationFailOpen,
			shell.persistentVoxelMaterialPresentationRows);
		Printf(
...
bool NRIRenderDevice::CreateRenderResources()
{
	...
	// Budget the expanded scene snapshots and all lazily initialized smoke sets.
	poolDesc.structuredBufferMaxNum = NRIStaticTangentStructuredPoolCapacity(NRIFrameShell::QueuedFrameCount);
	// Seven transient UAVs per queued smoke set, including the motion guide.
