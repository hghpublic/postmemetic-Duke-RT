...
Require-Match $runner '\$observationTransition = if \(\$TrailSideView\) \{ ''warptocoords -1700 560 -708 117 0; '' \} else \{ '''' \}[\s\S]*\+Fire; wait 8; -Fire; \$\{observationTransition\}\$\{postFire\}wait 1; screenshot' 'Trail side-view captures must move immediately after the eight-update RPG pulse to the flat-sector observer and take the first screenshot one update later.'
Require-Match $runner 'mode = ''trail-side-view''[\s\S]*launch = \[ordered\]@\{ position = @\(-1616, 760, -708\); yaw = 180; pitch = 0; sector = 306 \}[\s\S]*observation = \[ordered\]@\{ position = @\(-1700, 560, -708\); yaw = 117; pitch = 0; sector = 306; firstScreenshotWaitUpdates = 1 \}[\s\S]*geometryValidation = ''World Tour E1L1\.MAP point-in-polygon: both points are inside flat sector 306 \(ceiling -968, floor -580\)''[\s\S]*cameraValidation = ''geometry-safe camera candidate; screenshot review is required before art acceptance''' 'Trail side-view metadata must preserve the geometry-safe transforms and still require screenshot review.'
