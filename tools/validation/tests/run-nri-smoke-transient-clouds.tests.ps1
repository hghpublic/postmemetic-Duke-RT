...
$outputDir = Join-Path $root 'build/smoke-transient-cloud-tests'
$testExe = Join-Path $outputDir 'nri_smoke_transient_clouds.tests.exe'
...
$sources = @(
    (Join-Path $PSScriptRoot 'nri_smoke_transient_clouds.tests.cpp'),
...
if ($process.ExitCode -ne 0) { throw "transient cloud test compilation failed with exit code $($process.ExitCode)" }
& $testExe
if ($LASTEXITCODE -ne 0) { throw "transient cloud tests failed with exit code $LASTEXITCODE" }
