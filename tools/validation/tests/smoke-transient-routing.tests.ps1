...
Assert-Match $eventRunner 'warptocoords -1616 760 -708 180 0[\s\S]*position = @\(-1616, 760, -708\); yaw = 180; pitch = 0; sector = 306; recommendedEventDistanceWorldUnits = 160; blockingDistanceWorldUnits = 290' 'E1L1 event captures must record the map-derived open-roof sector and safe smoke distance.'
...
Assert-Match $source 'shape\.burstBorrow = burstBorrow &&\s*transientClass == LightOverlaySmokeTransientClass::Explosion' 'Only explicitly opted-in transient explosions may request burst borrowing.'
Assert-Match $source 'rule\.analyticCarrierCount, rule\.transientLobeCount, rule\.transientClass,\s*rule\.burstBorrow' 'Actor borrowing policy must reach the transient route.'
Assert-Match $source 'rule\.transientClass, rule\.burstBorrow, true, command\.count' 'Event borrowing policy must reach the transient route.'
