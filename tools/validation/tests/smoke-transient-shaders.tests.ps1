...
Require-Match $materialize 'float3\s+groupSource\s*=\s*0\.0;[\s\S]*groupSource\s*\+=\s*max\(externalSource\s*\+\s*intrinsicSource,\s*0\.0\);[\s\S]*source\s*\+=\s*min\(groupSource,\s*32\.0\)' 'Source radiance must be accumulated and clamped once per group, not once per lobe.'
