...
if ($videoOptions -notmatch 'Option\s+"Smoke",\s*"nri_ptsmoke",\s*"OnOff"\s*\r?\n\s*Option\s+"Smoke Quality",\s*"nri_ptsmokeworkprofile",\s*"NRISmokeWorkProfile"\s*\r?\n\s*CVarStaticText\s+"nri_settingsprofilewarning"\s*\r?\n\s*CVarStaticText\s+"nri_ptsmokemenuwarning"') {
    throw 'the smoke warning must occupy its own line beneath the ray-reconstruction profile warning'
}
if ($cvars -notmatch 'CVAR\(String, nri_ptsmokemenuwarning, "", 0\)[\s\S]*CUSTOM_CVAR\(Bool, nri_ptsmoke, true, CVAR_ARCHIVE \| CVAR_GLOBALCONFIG\)[\s\S]*nri_ptsmokemenuwarning\s*=\s*self[\s\S]*Smoke is very taxing on your GPU, especially at Ultra settings[\s\S]*:\s*""') {
